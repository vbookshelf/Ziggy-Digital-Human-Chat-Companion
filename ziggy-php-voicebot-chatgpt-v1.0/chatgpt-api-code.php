...
Your purpose is to demonstrate how voice recognition and text-to-speech can be used with ChatGPT.
EOT;
...
if (isset($_REQUEST["my_message"]) && empty($_REQUEST["robotblock"])) {
	...
	if (curl_errno($curl)) {
		...
		// Here we are checking if the doct has a key called 'error'.
			...
			// This "else" statement may not be necessary anymore because error will be 
			// captured in the above "else if" statement, when the token count is exceeded.
