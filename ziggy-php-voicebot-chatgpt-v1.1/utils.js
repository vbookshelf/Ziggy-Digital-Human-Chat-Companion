...
function saveChatHistoryToCsv() {
	...
	// Convert the json variable into a javascript variable
	var message_list = message_history;
  ...
  message_list.forEach((message) => {
	...
	// Add paragragh tags (<p>) so that
	// the text displays correctly when loaded from the saved csv file.
	// Note: This line is not in the javascript chatbot version.
	paragraph_text = formatResponse(message.content);
	
    rows.push([message.role, paragraph_text]);
  });
  ...
  link.setAttribute("download", `php_${timestamp}.csv`); //####### <-- File name
...
function writeCsvFileContentToPage(input_list) {
  ...
  function loadChatHistoryFromCsv(file) {
	  ...
	  reader.onload = function(event) {
		...
		// Send the javascipt varibale to the php server.
		// We need to set the chat_messages as the php session variable beause
		// the chat history is stored in this variable.
		var jsVariable = chat_messages;
	
		$.ajax({
		  url: "process_uploaded_csv_chat.php",
		  type: "POST",
